...
/**
 * per-app 刷新模式记忆表。
 * 数据模型: pkg -> { mode:int, lastUsed:long }
...
public final class PerAppStore {
    ...
    synchronized void put(String pkg, int mode) {
    ...
    synchronized void recordIfDirty(String pkg, int mode) {
        Entry e = cache.get(pkg);
        if (e != null && e.mode == mode && System.currentTimeMillis() - e.lastUsed < 60000) {
